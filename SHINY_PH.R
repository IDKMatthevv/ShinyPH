...
server <- function(input, output) {
  ...
  output$tabulka_vysledku <- renderTable({
    ...
    # Zjištění, zda jde o kyselinu nebo zásadu pro správné popisky
    oznaceni_K <- ifelse(data$vyber_AB == "A", "Ka", "Kb")
    oznaceni_pK <- ifelse(data$vyber_AB == "A", "pKa", "pKb")
    
    # Výpočet pK
    hodnota_pK <- -log10(data$konstanta)
    
    # Sestavení Data Framu pro tabulku
    data.frame(
      Parametr = c(
        "Typ látky",
        "Počáteční koncentrace c0 (mol/l)",
        paste("Disociační konstanta", oznaceni_K),
        oznaceni_pK,
        "Výsledné pH"
      ),
      Hodnota = c(
        data$typ,
        as.character(data$c0),
        as.character(data$konstanta),
        as.character(round(hodnota_pK, 2)),
        as.character(round(data$ph, 2))
      )
    )
  }, colnames = FALSE)
